...
extern double test2(float *__restrict a, float *__restrict b, float *__restrict c, int N);
...
int main(int argc, char * argv[]) {
  ...
  switch (whichTestToRun) {
    case 1: 
      for(int run_time = 0; run_time < n; run_time++){
        time_set[run_time] = test1(values1, values2, output, N);
      }
      break;
    case 2: 
      for(int run_time = 0; run_time < n; run_time++){
        time_set[run_time] = test2(values1, values2, output, N);
      }
      break;
    case 3: 
      test3(values3, N);
  ...
  for(int i = 0; i < n; i++){
    cout << i << " time : " << time_set[i] << endl;
  }

  // find the median
  std::sort(time_set, time_set + n);

  for(int i = 0; i < n; i++){
    cout << i << " time : " << time_set[i] << endl;
  }

  cout << "median time: " << time_set[n/2] << endl;
